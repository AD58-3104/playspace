...
namespace Citbrains
{
    namespace infosharemodule
    {
        ...
        public:
            ...
            #else
            static const int32_t NUM_PLAYERS = 6;
            static const int32_t COMM_INFO_PORT0 = 7110; //!< CommInfoで使用するPORTのはじめのポート
                                                                    //1:7110, 2:7111, 3:7112, 4:7113, 5:7114, 6:7115

            static const int32_t MAX_COMM_INFO_OBJ = 7;    //!< 共有するオブジェクトの最大値(はじめはボール)
            static const int32_t MAX_STRING = 42;          //!< メッセージの最大値
            static const int32_t MAX_BEHAVIOR_STRING = 32; //この三つは多分もういらない

            static const uint8_t COMM_NOT_EXIST = 0b0000'0000;     //!< 発見しないときの値
            static const uint8_t COMM_EXIST = 0b0000'0001;         //!< 発見しかたどうかのフラグ
            static const uint8_t COMM_OUR_SIDE = 0b0000'0010;      //!< 自チームかどうかのフラグ
            static const uint8_t COMM_OPPOSITE_SIDE = 0b0000'0100; //!< 敵チームかどうかのフラグ

            static const int32_t MAX_BLACK_POLES = 6; //!< 黒ポールの数
            static const int32_t MAX_TARGET_POS = 1;
            static const int32_t MAX_MAGENTA_OBJECTS = 3;                //!< マゼンタロボットの数
            static const int32_t MAX_CYAN_OBJECTS = MAX_MAGENTA_OBJECTS; //!< シアンロボットの数

            static const int32_t NO_MAGENTA = 0;    //!< マゼンタの番号(色の番号とは異なる)
            static const int32_t NO_CYAN = 1;       //!< シアンの番号
            #endif
                                                               //---------------------------------------------------------------------------
